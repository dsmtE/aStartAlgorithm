...
bool AStar::next() {
    if(!done_) { // si on à déjà fini on faire rien de plus
        ...
        if (!openList_.empty()) {
            ...
            // si on est toujours pas arrivé à notre destination on continue sinon on s’arrête ici
            if (current->pos_ != B_) {
                ...
                for (int i = 0; i < (moveDiagonally_ ? 8 : 4); i++) {
                    ...
                    // on check si notre nouvelle position à tester existe sur la grille et si elle n'est pas un obstacle (qu'il n'y a pas collision)
                    if( !isInBound(newPos, rows_, cols_) || isCollisionAt(newPos) ) {
                        // si c'est l'un des deux cas ou les deux cela ne sert à rien de continuer avec celle-ci, on passe à la position suivante à tester
                        continue;
                    }
...
std::vector<glm::ivec2> AStar::computePath() {
    ...
    while (!openList_.empty()) {
        // on recupère la cellule avec le coût le plus faible
        current = openList_.top();
        // si on est arrivé à notre destination, on s’arrête ici
        if (current->pos_ == B_) {
            break;
        }
    ...
    // si on est effectivement arrivé à la destination B
    if (current != nullptr && current->pos_ == B_) {
        while(current != nullptr) {
            // on parcours les cellules une à une grâce aux pointeurs vers leurs parents pour remonter le fil du chemin
            path_.push_back(current->pos_);
            current = current->cameFrom_;
        }
        // on inverse l'ordre pour avoir le départ à A
        std::reverse(path_.begin(), path_.end());
        done_ = true;
    } else {
        // si on est pas à la destination mais qu'il n'y a plus de cellules à parcourir, le chemin n'existe pas 
        std::cout << "path can't be found" << std::endl;
    }
    return path_;
}
